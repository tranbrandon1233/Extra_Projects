  ...
  const observer = new IntersectionObserver((entries) => {
    ...
    if (entries[0].isIntersecting) {
      ...
      callback().then((newContent) => {
        // Append the new content to the container before the load more element
        newContent.forEach((item) => {
          container.insertBefore(item, loadMore);
        });

        // Add a spacer element to create space between the new content and the load more element
        const spacer = document.createElement('div');
        spacer.style.height = '20px'; // Adjust the height as needed
        container.insertBefore(spacer, loadMore);
